...
Output **ONLY THE CODE** and **NO MARKDOWN BACKTICKS**"""
    ...
    def _execute_tool(self, code: str) -> str:
        ...
        # Create a safe globals dict with only the allowed tools
        globals_dict = {
        ...
        try:
            # Clean up the code and validate it's a simple function call
            code = code.strip()
            if not any(code.startswith(f"{name}(") for name in globals_dict):
                raise ToolExecutionError(f"Invalid tool call format. Must be a direct call to one of: {', '.join(globals_dict.keys())}")
            
            # Log the tool being called
            logger.debug(f"Executing tool call: {code}")
            ...
            try:
                logger.debug(f"Tool call result: {str(result)[:200]}...")
            except Exception:
                logger.debug("Tool call completed but result could not be logged")
    ...
    def stream(self, messages_dict: Dict[str, List[Any]], config: Dict[str, Any] = None) -> Generator[Dict[str, Any], None, None]:
        ...
        max_retries = 3  # Maximum retries for the same tool call
        
        # Check if this is a direct conversation (no tools provided)
        is_conversation = not self.tools
        
        while True:
            try:
                # Build and add prompt to history
                ...
                # Get trimmed history and invoke model
                full_history = self._trim_chat_history(initial_messages, chat_history)
                response = self.model.invoke([
                    SystemMessage("Respond naturally as a helpful AI assistant." if is_conversation else "Execute efficiently yet completely as a fully autonomous agent.")
                ] + full_history)
                ...
                if is_conversation:
                    # For direct conversations, just yield the response
                    yield {"content": response.content}
                    break  # End the conversation after one response
                else:
                    try:
                        # Execute the tool call
                        ...
                        yield {}  # Success case yields empty dict
                        ...
                        chat_history.append(HumanMessage(
                            content=f"Your tool call caused an error: {e}\n\n"
                                   f"Available tools: {', '.join(t.func.__name__ for t in self.tools)}\n\n"
                                   f"Please correct your tool call and try again."
                        ))
                        
                        # Yield error chunk for UI
                        yield self._create_error_chunk(str(e))
                        
                        # Check if we should retry
                        if len(chat_history) > max_retries * 2:  # Each iteration adds 2 messages
                            raise RuntimeError(f"Agent failed to execute tools successfully after {max_retries} attempts")
                
            except Exception as e:
                # Handle any other errors (model errors, etc)
